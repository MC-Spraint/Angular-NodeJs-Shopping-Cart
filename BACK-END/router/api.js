...
router.post('/register' ,async (req,res,next)=>{
	...
	rand=Math.floor((Math.random()*100)+54);                                                host=req.get('host');
        ...
        smtpTransport.sendMail(mailOptions, async function(error,response){             
			...
			res.json("A link has been sent to your email,click on that link to verify and create your account successfully!");
...
router.get('/verify',async function(req,res,next){
	...
	if((req.protocol+"://"+req.get('host'))==(prot+"://"+host)){
		...
		if(req.query.id==rand){   
			...
			//res.status(200).json("Congratulation!<br/>Welcome to Eazykart.<br/>Your account with email "+mailOptions.to+" is created successfully....");     
		console.log("Email is verified.");
			}
			catch(error){
			res.status(401).json("Failed to create the account");    
		
			}           
		}
		else{                                                                                           res.status(400).json("<h1>Bad Request</h1>");
			console.log("Email is not verified.")                                              }
	}
	else{                                                                                          res.status(403).json("<h1>Request is from unknown source");
		}
	});
...
router.post('/login' ,async (req,res,next)=>{
       const user_presence = await userModel.findOne({email:req.body.email});               if(user_presence){
			...
			/*  req.session.user_presence=user_presence;
                ...
                else{                                                                                           res.status(401).json("Incorrect password!");                                                        console.log("Incorrect password!");                                             }                                                                               }                                                                           else{                                                                                           res.status(401).json("Incorrect email id!");                                                        console.log("Incorrect email id!");                                             }
